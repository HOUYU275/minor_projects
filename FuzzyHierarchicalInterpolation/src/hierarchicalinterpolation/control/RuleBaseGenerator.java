...
public class RuleBaseGenerator {
    ...
    private float minConsequenceRange = Float.MAX_VALUE;
    private float maxConsequenceRange = Float.MIN_VALUE;
    ...
    public Observation newObservation(int numberOfVariables, double[] ranges) {
        ...
        observationAntecedents[0] = new TriangularFuzzyNumber(a1 - 0.9, a1 + 0.1, a1 + 1.1);
        observationAntecedents[1] = new TriangularFuzzyNumber(a2 - 0.9, a2 + 0.1, a2 + 1.1);
        observationAntecedents[2] = new TriangularFuzzyNumber(a3 - 0.9, a3 + 0.1, a3 + 1.1);
    ...
    public double[] getCrispInputs(int numberOfVariables, Observation observation) {
        ...
        for (int i = 0; i < crispInput.length; i++) {
            crispInput[i] = observation.getAntecedents()[i].getRepresentativeValue();
        }
        return crispInput;
    }

    private void assignValueRanges(float[][] antecedentCentroids) {

        minAntecedentRanges = new float[antecedentCentroids.length];
        maxAntecedentRanges = new float[antecedentCentroids.length];

        for (int i = 0; i < antecedentCentroids.length; i++) {
            minAntecedentRanges[i] = Float.MAX_VALUE;
            maxAntecedentRanges[i] = Float.MIN_VALUE;
    ...
    public ArrayList<Rule> generateRules(TestFunction testFunction, int numberOfFuzzySetsPerVariable) {
        ...
        float[][] antecedentCentroids = new float[testFunction.getNumberOfVariables()][numberOfFuzzySetsPerVariable];
        float[] consequenceCentroids = new float[numberOfRules];
        Random random = new Random();
        ...
        while (rules.size() < numberOfRules) {
            float[] input = new float[testFunction.getNumberOfVariables()];
            FuzzyNumber[] fuzzyInput = new FuzzyNumber[testFunction.getNumberOfVariables()];

            for (int i = 0; i < testFunction.getNumberOfVariables(); i++) {
                fuzzyInput[i] = new TriangularFuzzyNumber(
                        ...
                        roundToOneDecimal(antecedentCentroids[i][currentIndices[i]] + random.nextDouble() * 1));
                input[i] = fuzzyInput[i].getRepresentativeValue();
            }

            consequenceCentroids[rules.size()] = roundToOneDecimal(testFunction.calculate(input));
            FuzzyNumber fuzzyResult = new TriangularFuzzyNumber(
        ...
        for (int i = 0; i < numberOfRules; i++) {
            rules.get(i).setMaxConsequenceRange(maxConsequenceRange + 1f);
            rules.get(i).setMinConsequenceRange(minConsequenceRange - 1f);
        }
    ...
    public ArrayList<Rule> generate(TestFunction testFunction, int numberOfInput) {
        ArrayList<Rule> rules = new ArrayList<Rule>();
        float[][] repValues = new float[testFunction.getNumberOfVariables()][numberOfInput];
        ...
        for (int i = 0; i < testFunction.getNumberOfVariables(); i++) {
            ...
            for (int j = 0; j < numberOfInput; j++) {
                ...
                repValues[i][j] = (float)(min + spread * j + random.nextInt(spread));
        ...
        for (int j = 0; j < 38; j++) {
            ...
            for (int i = 0; i < testFunction.getNumberOfVariables(); i++) {
                ...
                fuzzyInput[i] = new TriangularFuzzyNumber(input[i] - 1d, input[i], input[i] + 1d);
            }
            ...
            FuzzyNumber fuzzyResult = new TriangularFuzzyNumber(leftResult[j], result[j], rightResult[j]);
        ...
        float[] input38 = new float[testFunction.getNumberOfVariables()];
        FuzzyNumber[] fuzzyInput = new FuzzyNumber[testFunction.getNumberOfVariables()];
        for (int i = 0; i < testFunction.getNumberOfVariables(); i++) {
            input38[i] = repValues[i][0];
            fuzzyInput[i] = new TriangularFuzzyNumber(input38[i] - 1d, input38[i], input38[i] + 1d);
        }
        ...
        FuzzyNumber fuzzyResult = new TriangularFuzzyNumber(leftResult[38], result[38], rightResult[38]);
        Rule rule38 = new Rule(fuzzyInput, fuzzyResult, testFunction.getMaxAntecedentRanges(),
                testFunction.getMinAntecedentRanges(), testFunction.getMaxConsequenceRange(),
                testFunction.getMinConsequenceRange());
        rules.add(rule38);

        float[] input39 = new float[testFunction.getNumberOfVariables()];
        FuzzyNumber[] fuzzyInput39 = new FuzzyNumber[testFunction.getNumberOfVariables()];
        for (int i = 0; i < testFunction.getNumberOfVariables(); i++) {
            input39[i] = repValues[i][numberOfInput - 1];
            fuzzyInput39[i] = new TriangularFuzzyNumber(input39[i] - 1d, input39[i], input39[i] + 1d);
        }
        ...
        FuzzyNumber fuzzyResult39 = new TriangularFuzzyNumber(leftResult[39], result[39], rightResult[39]);
    ...
    private float roundToOneDecimal(double d) {
