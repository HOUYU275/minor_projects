...
public class Status extends WeiboResponse implements java.io.Serializable {
	...
	private void constructJson(JSONObject json) throws WeiboException {
		try {
			...
			if(!json.isNull("pic_urls")){
                JSONArray array = json.getJSONArray("pic_urls");
                pic_urls = new String[array.length()];
                for (int i = 0; i < array.length(); i++) {
                    pic_urls[i] = array.getJSONObject(i).getString("thumbnail_pic");
                }
			}
	...
	public String toString() {
        String append = "";
        for (String s : pic_urls) append += s + "; ";
		return "Status [user=" + user + ", idstr=" + idstr + ", createdAt="
				...
				+ ", visible=" + visible + ", pic_urls=" + append + "]";
	}

    public String[] getPictureURLs() {
        if (pic_urls == null || pic_urls.length == 0) return null;
        String[] urls = new String[pic_urls.length];
        for (int i = 0; i < pic_urls.length; i++)   {
            urls[i] = pic_urls[i].replace("thumbnail", "large");
        }
        return urls;
    }
