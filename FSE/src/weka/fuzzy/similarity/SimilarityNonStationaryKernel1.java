 ...
 *
 */
public class SimilarityNonStationaryKernel1
  extends Similarity
   ...
   */
  public SimilarityNonStationaryKernel1() {
    super();
  }
   ...
   */
  public SimilarityNonStationaryKernel1(Instances data) {
    super(data);
  }
  ...
  public String globalInfo() {
    return 
      ...
      + "(abs(a(x)) +abs(a(y))-abs(a(x)-a(x))) / 2sqrt(abs(a(x)) *abs(a(y))).";
  }

  public String toString() {
	  return "(abs(a(x)) +abs(a(y))-abs(a(x)-a(x))) / 2sqrt(abs(a(x)) *abs(a(y))).";
  }
 ...
 //to be filled in
  ...
  public double similarity(int index, double first, double second) {
      if (Math.abs(first) * Math.abs(second) == 0) {
          return 0;
      }          
      else {
          return (Math.abs(first) + Math.abs(second) - Math.abs(first - second)) / (2 * Math.sqrt(Math.abs(first) * Math.abs(second)));
      }
    ...
    public double[] similarity(int index, double first, double second, double param) {
        return new double[0];  //TODO: Automatically Generated Implemented Method
    }
