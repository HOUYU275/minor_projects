...
public class UWeakGamma extends UnsupervisedFuzzyMeasure  implements TechnicalInformationHandler {
	...
	public double calculate(BitSet subset, int decAttr) {
		...
		for (int x = 0; x < m_numInstances; x++) {				
			...
			for (int y = 0; y < m_numInstances; y++) {
				...
				currLower = m_Implicator.calculate(condSim, fuzzySimilarity(decAttr, x, y));
	...
	public String globalInfo() {
		return "Unsupervised weak gamma evaluator, using the fuzzy rough lower approximations.\n\n"
		+  getTechnicalInformation().toString();
	}

	public String toString() {
		return "Unsupervised Weak Gamma";
	}
}
