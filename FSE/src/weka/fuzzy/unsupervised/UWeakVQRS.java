...
public class UWeakVQRS extends UnsupervisedFuzzyMeasure  implements TechnicalInformationHandler {
	...
	public double calculate(BitSet subset, int decAttr) {
		...
		for (int x = 0; x < m_numInstances; x++) {
			...
			val = lowerVQRS(current, x, x, decAttr);
	...
	public String globalInfo() {
		return "Unsupervised Weak VQRS evaluator, using the vaguely quantified fuzzy rough lower approximations.\n\n"
		+ toString() + "\n\n" + getTechnicalInformation().toString();
	}

	public String toString() {
		return "Unsupervised Weak VQRS: alpha="+alpha+" beta="+beta;
	}
}
