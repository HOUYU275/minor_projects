...
public class UDiscernibilityG extends UnsupervisedFuzzyMeasure implements TechnicalInformationHandler {
	...
	public UDiscernibilityG() {
		super();
	}
	
	
	@Override
	public double calculate(BitSet reduct, int decAttr) {
		double difd;
		double disc, d;
		double ret;	
		ret = 0;
		...
		for (int i = 0; i < m_numInstances - 1; i++) {
			for (int j = i + 1; j < m_numInstances; j++) {
				...
				difd = fuzzySimilarity(decAttr, i, j);
	...
	public String toString() {
		return "Unsupervised Discernibility function - g";
	}
}
