...
public abstract class FuzzyDiscernibilityMeasure extends FuzzyMeasure implements Serializable {
	...
	private void calculateMatrix() {
		...
		for (int i = 0; i < m_numInstances; i++) {
			for (int j = i + 1; j < m_numInstances; j++) {
				...
				for (int a=0;a<m_numAttribs;a++) {
					...
					if (a!=m_classIndex) {
						sum+=values[a];
						//if (nonZeroIndex<0 && values[a]>0) nonZeroIndex=a;
						
						max = m_SNorm.calculate(max, values[a]); 
						
						if (values[a]>0) {
							setValues++; // == fuzzy discernibility>0
						}
		...
		/*Iterator<Clause> i = clauseList.iterator();
		double[] count = new double[m_numAttribs];
		while (i.hasNext()) {
			Clause o = i .next();

			for (int a=0;a<m_numAttribs;a++) {
				if (o.values[a]==o.max) count[a] ++;
			}
		}
		...
		for (int a=0;a<m_numAttribs;a++) {
			if (a!=m_classIndex && count[a]==clauseList.size()) removed++; 
		}

		System.err.println("Can remove "+removed+" attributes");*/
