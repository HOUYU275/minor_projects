...
public class PSOSearch
     ...
     * the lookup table
     */
    private Hashtable<BitSet, Particle> m_lookupTable;
    ...
    public String[] getOptions() {
        ...
        String[] options = new String[15 + heuristicOptions.length + 1];
        int current = 0;
    ...
    public int[] search(ASEvaluation ASEval, Instances data)
        ...
        try{
            ...
            m_lookupTable = new Hashtable<BitSet, Particle>(m_lookupTableSize);
            ...
            m_bestMerit = m_gBest.getObjective();
        ...
        System.out.println("\t" + (System.currentTimeMillis() - startTime) + "\t" + selectedAttributes.length + "\t" + m_bestMerit);*/
            ...
            return attributeList(m_gBest.getSubset());
        }
        return attributeList(m_gBest.getSubset());
    }
    ...
    private void updateParticle(Particle p) {
        ...
        //if (vel<=gdistance) {
        for (int a = 0; a < vel; a++) {
            int val = m_random.nextInt(m_numAttribs);
            if (val != m_classIndex) {
                if (m_gBest.get(val)) {
                    if (p.subset.get(val)) {
                    }//a--;
    ...
    private double evaluateParticle(SubsetEvaluator ASEvaluator, Particle particle)
        ...
        if (m_lookupTable.containsKey(particle.getSubset()) == false) {
            merit = ASEvaluator.evaluateSubset(particle.getSubset());
            if (merit > 1) merit = 1;
            particle.setObjective(merit);
            m_lookupTable.put(particle.getSubset(), particle);
            ...
            Particle temp = (Particle) m_lookupTable.get(particle.getSubset());
    ...
    private final void randomReduct(BitSet s) {
        ...
        for (int a = 0; a < numAttrs; a++) {
            val = m_random.nextInt(m_numAttribs);
