 ...
 */
public class FeatureVoteHarmonySearch extends ASSearch
    ...
    private FeatureVoteHarmonyMemory featureVoteHarmonyMemory;
     ...
     * Constructor
     */
    public FeatureVoteHarmonySearch() {
        resetOptions();
    }
    ...
    public int[] search(ASEvaluation ASEval, Instances data)
        ...
        featureVoteHarmonyMemory = new FeatureVoteHarmonyMemory(memorySize);
...
/*            temp = generateRandomSubset();
           ...
           for (FeatureSubset fts : featureSubsetHarmonyMemory) {
               System.out.println(rank + fts.toString() + " - " + fts.getMerit() + " - " + featureSubsetHarmonyMemory.add(fts));
               rank ++;
           }
           ...
           return attributeList(featureSubsetHarmonyMemory.last());*/
            ...
            FeatureVote fs = featureVoteHarmonyMemory.newHarmony();
            BitSet bs = fs.toBitSet();
            fs.setMerit(ASEvaluator.evaluateSubset(bs));
            fs.setCardinality(bs.cardinality());
            ...
            if (added) {
                ...
                lastAdded = m;
                if (featureVoteHarmonyMemory.comparator().compare(featureVoteHarmonyMemory.last(), fs) == 0) {
                    optimalAdded = m;
                }
            } else {
        ...
        int rank = 1;
        for (FeatureVote fv : featureVoteHarmonyMemory) {
    ...
    private FeatureVote generateRandomFeatureVote() {
        FeatureVote fs = new FeatureVote(ensembleSize);
    ...
    protected class FeatureVoteHarmonyMemory extends TreeSet<FeatureVote> {
        ...
        public boolean add(FeatureVote e, Boolean bypass) {
        ...
        public FeatureVote newHarmony() {
            FeatureVote newFS = new FeatureVote(ensembleSize);
            for (int i = 0; i < ensembleSize; i++) {
