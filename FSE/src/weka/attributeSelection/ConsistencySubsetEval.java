 ...
 * @see weka.filters.supervised.attribute.Discretize
 */
public class ConsistencySubsetEval
        extends ASEvaluation
        implements SubsetEvaluator,
        TechnicalInformationHandler, InterruptingEvaluator, CachedEvaluator {
    ...
    private boolean willInterrupt = true;

    @Override
    public void interrupt() throws MaximumEvaluationReachedException {
        if (willInterrupt) {
            evaluationCounter++;
            if (evaluationCounter > maximumEvaluation) throw new MaximumEvaluationReachedException();
        }
    }
    ...
    public double evaluateSubset(BitSet subset) throws Exception {
        //if (cache.contains(subset)) return cache.get(subset);
        //interrupt();
        int[] fs;
        int i;
        int count = 0;
        ...
        //cache.put(subset, consistencyCount());
