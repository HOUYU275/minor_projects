 ...
 */
public class HillClimberWithClassifier
extends ASSearch
implements RankedOutputSearch, StartSetHandler, OptionHandler {
	...
	protected int m_numAttribsSelect;
	 ...
	 * Constructor
	 */
	public HillClimberWithClassifier () {
	...
	throws Exception {
		...
		while (!done) {
			...
			if (addone) {
				...
				int[] m_SelectedAttributes = new int[m_best_group.cardinality()+1];
				int jj = 0;
				int ii = 0;

				for ( ii = 0, jj = 0; ii < m_numAttribs; ii++) {
					if (m_best_group.get(ii)) {
						m_SelectedAttributes[jj++] = ii;
					}
				}

				m_SelectedAttributes[jj] = m_classIndex;

				Instances copy = new Instances(data);

				m_attributeFilter = new Remove();
				m_attributeFilter.setAttributeIndicesArray(m_SelectedAttributes);
				m_attributeFilter.setInvertSelection(true);
				m_attributeFilter.setInputFormat(copy);

				copy = Filter.useFilter(copy, m_attributeFilter);

				Evaluation m_Evaluation = new Evaluation(copy);
			    m_Evaluation.crossValidateModel(m_BaseClassifier, copy, 10, new Random(1));
			    double error = m_Evaluation.errorRate();

			    if (data.attribute(m_classIndex).isNominal()) System.err.println("Accuracy: "+(1-error));
			    else System.err.println("RMSE: "+error);
			    if (!m_backward&&best_merit>=alpha) {
					done=true;
					break;
				}
