 ...
 */
public class HarmonySearchFixed extends ASSearch
...

{
    ...
    public int[] search(ASEvaluation ASEval, Instances data)
        ...
        //(new CrossValidation()).validate(new J48(), data, AttributeLists.getAttributeLists(8));
        //(new CV()).validate(new FuzzyNN(), data, AttributeLists.getAttributeLists(8));
        //System.out.println();
        //}
        ...
        FeatureSelectionHarmonyMemory featureSelectionHarmonyMemory =
                new FeatureSelectionHarmonyMemory(HarmonyMemory.createParameterRanges(
                        numHarmonies, numHarmonies,
                        HMCR, HMCR,
                        1, 1,
                        0, 0),
                        valueRanges, m_random, subsetEvaluator);
        //FeatureSelectionHarmonyMemory featureSelectionHarmonyMemory = new FeatureSelectionHarmonyMemory(HarmonyMemory.createParameterRanges(numHarmonies, HMCR), valueRanges, m_random, subsetEvaluator);
        featureSelectionHarmonyMemory.fill();
        featureSelectionHarmonyMemory.iterate(iteration);

        long endTime = System.currentTimeMillis();

        m_bestMerit = featureSelectionHarmonyMemory.best().getMerit(0);
        int[] selectedAttributes = ((FeatureSelectionHarmonyComparator) featureSelectionHarmonyMemory.getHarmonyComparator()).toArray(featureSelectionHarmonyMemory.best());

        for (int attribute : selectedAttributes) System.out.print(attribute + " ");
        System.out.println("\t" + (System.currentTimeMillis() - startTime) + "\t" + selectedAttributes.length + "\t" + m_bestMerit);
