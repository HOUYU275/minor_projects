...
public class AntSearch
     ...
     */
    private ASEvaluation m_HeuristicEvaluator = new CfsSubsetEval();
    ;
    ...
    public int[] search(ASEvaluation ASEval, Instances data)
        ...
        m_numAnts = m_numAttribs < 40 ? 20 : (m_numAttribs / 2);
        m_maxGenerations = m_numAttribs < 40 ? 20 : (m_numAttribs / 2);
        ...
        //System.out.println(((SubsetEvaluator) ASEval).evaluateSubset(m_best.subset));

        int[] selectedAttributes = attributeList(m_best.getSubset());

        /*for (int attribute : selectedAttributes) System.out.print(attribute + " ");
        System.out.println("\t" + (System.currentTimeMillis() - startTime) + "\t" + selectedAttributes.length + "\t" + best_merit);*/
            ...
            if (a != m_classIndex) {
                ...
                for (int j = a + 1; j < m_numAttribs; j++) {
                    if (j != m_classIndex) {
                        ...
                        bs.set(j);
                        if (attributeEval)
                            heuristic.setCell(j, a, attributeEvaluator.evaluateAttribute(a) + attributeEvaluator.evaluateAttribute(j));
    ...
    private double evaluateAnt(SubsetEvaluator ASEvaluator, Ant ant)
        ...
        if (m_lookupTable.containsKey(ant.getSubset()) == false) {
            ...
            Ant temp = (Ant) m_lookupTable.get(ant.getSubset());
