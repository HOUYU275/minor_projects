...
public class MemeticSearch
    ...
    protected class GABitSet
            implements Cloneable, Serializable, RevisionHandler, Comparable {
		...
		public int compareTo(Object o) {
			if (this.m_fitness > ((GABitSet) o).getFitness()) return 1;
			if (this.m_fitness < ((GABitSet) o).getFitness()) return -1;
			return 0;
		}
	}
    ...
    public int[] search(ASEvaluation ASEval, Instances data)
        ...
        try {
            ...
            for (int i = 1; i <= m_maxGenerations; i++) {
                m_population = sort(m_population);
                m_population[m_popSize - 1] = localSearch(m_population[m_popSize - 1]);
                m_population[m_popSize - 2] = localSearch(m_population[m_popSize - 2]);
            /*quickLocalSearch(m_population[m_popSize - 1]);
            quickLocalSearch(m_population[m_popSize - 2]);*/
                generation();
                evaluatePopulation(ASEvaluator);
                populationStatistics();
                scalePopulation();
            ...
            localSearch(true);
            //System.out.println(evaluator.evaluateSubset(m_best.getChromosome()) + " ");
        }
        catch (MaximumEvaluationReachedException e) {
            return NatureInspiredCommon.toIntArray(m_best.getChromosome(), m_numAttribs);
        }
        return NatureInspiredCommon.toIntArray(m_best.getChromosome(), m_numAttribs);
    }

    private void quickLocalSearch(GABitSet bitSet) throws Exception {
        BitSet temp = (BitSet) bitSet.getChromosome().clone();
        double score = bitSet.getFitness();
        double tempScore, bestDifference = -Double.MAX_VALUE;
        int bestIndex = -1;
        int iterations = m_random.nextInt(m_numAttribs);
        ...
        if (bestDifference > 0) bitSet.getChromosome().flip(bestIndex);
    }

    private void localSearch(boolean improveOnly) throws Exception {
        //if (evaluator instanceof InterruptingEvaluator) ((InterruptingEvaluator)evaluator).pause();
        double recordScore = m_best.getObjective();
        BitSet temp = (BitSet) m_best.getChromosome().clone();
        double tempScore = 0d, bestDifference = -Double.MAX_VALUE, bestScore = m_best.getObjective();
        int addIndex = -1;
        //add
        for (int i = m_best.getChromosome().nextClearBit(0); i < m_classIndex - 1; i = m_best.getChromosome().nextClearBit(i + 1)) {
            if (i == -1) break;
            temp.set(i);
            tempScore = evaluator.evaluateSubset(temp);
        ...
        if ((bestDifference > 0) || !improveOnly) temp.set(addIndex);
        int deleteIndex = -1;
        tempScore = 0d;
        bestDifference = -Double.MAX_VALUE;
        bestScore = evaluator.evaluateSubset(temp);
        //delete
        for (int i = m_best.getChromosome().nextSetBit(0); i < m_classIndex; i = m_best.getChromosome().nextSetBit(i + 1)) {
            if (i == -1) break;
            temp.clear(i);
            tempScore = evaluator.evaluateSubset(temp);
            if (tempScore - bestScore > bestDifference) {
        ...
        if ((bestDifference > 0) || !improveOnly) temp.clear(deleteIndex);
        //if (evaluator instanceof InterruptingEvaluator) ((InterruptingEvaluator)evaluator).unpause();
        //System.out.println(recordScore + " -> " + evaluator.evaluateSubset(temp) + " +" + addIndex + " -" + deleteIndex);
        m_best.setChromosome(temp);
    }

	private GABitSet localSearch(GABitSet bitSet) throws Exception {
        //if (evaluator instanceof InterruptingEvaluator) ((InterruptingEvaluator)evaluator).pause();
		BitSet temp = (BitSet) bitSet.getChromosome().clone();
		double tempScore = 0d, bestDifference = -Double.MAX_VALUE, bestScore = m_best.getObjective();
		int addIndex = -1;
		//add
		for (int i = bitSet.getChromosome().nextClearBit(0); i < m_numAttribs; i = bitSet.getChromosome().nextClearBit(i + 1)) {
			if (i == -1) break;
			temp.set(i);
			tempScore = evaluator.evaluateSubset(temp);
			if (tempScore - bestScore > bestDifference) {
		...
		if (bestDifference < 0) return bitSet;
		bitSet.set(addIndex);
		int deleteIndex = -1;
		bestDifference = -Double.MAX_VALUE;
		bestScore = evaluator.evaluateSubset(temp);
		//delete
		for (int i = bitSet.getChromosome().nextSetBit(0); i < m_classIndex; i = bitSet.getChromosome().nextSetBit(i + 1)) {
			if (i == -1) break;
			temp.clear(i);
			tempScore = evaluator.evaluateSubset(temp);
			if (tempScore - bestScore > bestDifference) {
		...
		if (bestDifference > 0) bitSet.clear(deleteIndex);
		bitSet.setFitness(evaluator.evaluateSubset(bitSet.getChromosome()));
        //if (evaluator instanceof InterruptingEvaluator) ((InterruptingEvaluator)evaluator).unpause();
		return bitSet;
	}

	private GABitSet[] sort(GABitSet[] input) {
		GABitSet[] output = new GABitSet[input.length];
		Vector<GABitSet> vector = new Vector<>(Arrays.asList(input));
		Collections.sort(vector);
		for (int i = 0; i < vector.size(); i++) output[i] = vector.get(i);
		return output;
	}
