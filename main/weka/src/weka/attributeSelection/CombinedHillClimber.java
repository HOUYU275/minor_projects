 ...
 */
public class CombinedHillClimber
extends ASSearch
implements StartSetHandler, OptionHandler {
	 ...
	 * A threshold
	 */
	protected double m_threshold=1;
		...
		while (!done) {
			...
			for (i=0;i<m_numAttribs;i++) {
				...
				if (z) {
					...
					if (m_backward) {
						z = (tempEval[0] >= temp_merit[0]);
						z2 = (tempEval[1] >= temp_merit[1]);
					} else {
			...
			if (addone) {
				double denom1=-1,denom2=-1;
				int attrIndex=-1;
				String choice="";

				if (temp_merit[0]!=0 && temp_merit[1]!=0) {
					denom1=temp_merit[0];
					denom2=temp_merit[1];
				}

				double val1 = m_threshold*best_prev[0]/denom1;
				double val2 = best_prev[1]/denom2;

				if (val1==val2) {
					if (best_prev[0]>best_prev[1]) {
						best_merit = temp_merit[0];
						attrIndex = temp_index[0];
						choice=mes1;
					}
					else {
						best_merit = temp_merit[1];
						attrIndex = temp_index[1];
						choice=mes2;
					}
				}
				else if (val1<val2) {
				...
				if (m_backward) {
					m_best_group.clear(attrIndex);
					searchOutput.append(m_best_group+" => "+best_merit+"\n");
					System.err.println(m_best_group+" => "+best_merit);
				} else {
					m_best_group.set(attrIndex);
					//double gamma = ((SubsetEvaluator)m_evaluator1).evaluateSubset(m_best_group);

					System.err.println(mes1+": "+val1);
					System.err.println(mes2+": "+val2);
					System.err.println("Choosing "+choice+"\n");
					searchOutput.append(m_best_group+" => "+best_merit+"\n");
					System.err.println(m_best_group+" => "+best_merit);
				...
				best_prev[0] = temp_merit[0];
				best_prev[1] = temp_merit[1];
