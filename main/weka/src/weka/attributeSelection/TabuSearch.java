 ...
 * Tabu Search :<br/>
 * <br/>
 * Performs a search  through the space of attribute subsets. Evading local maximums by accepting bad and diverse solutions and make further search in the best solutions. Stops when there's not more improvement in n iterations<br/>
...
public class TabuSearch extends ASSearch
     ...
     * @throws Exception if the evaluation can not be complete
     */
    private List<Subset> generateNeighborhood(BitSet S, int numNeighborhood)
        ...
        if (numNeighborhood >= m_numAttribs) {
     ...
     * @throws Exception if any evaluation can't be completed
     */
     ...
     * @throws Exception if the evaluation can not be completed
     */
    public Subset GenerateInitialSolution(Subset initial, int size)
        ...
        m_Sbest = initial.clone();

        while (m_Sbest.cardinality() < size) {

            Subset tempSubset = new Subset();
            double bestMerit = m_Sbest.merit;
            int bestIndex = -1;

            for (int i = 0; i < rankedAttribsCopy.size(); i++) {
                Subset candidate = ((Subset) rankedAttribsCopy.get(i)).clone();
                if (m_Sbest.subset.get(candidate.subset.nextSetBit(0)))
                    continue;

                tempSubset = joinSubsets(m_Sbest, rankedAttribsCopy.get(i));
            ...
            m_Sbest = joinSubsets(m_Sbest, rankedAttribsCopy.get(bestIndex));
     ...
     * @throws Exception if the evaluation of the subsets can not be completed
     */
    ...
    public String printSubset(Subset subset) {
        ...
        //System.out.println (" with a merit of: "+merit);
