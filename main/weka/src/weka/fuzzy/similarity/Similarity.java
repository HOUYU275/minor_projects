...
/**
 * Represents the abstract ancestor for similarity based distance functions.
 *
...
implements DistanceFunction, OptionHandler, Serializable {
	...
	/** The cutoff value used: when the similarity is lower than this value, calculations are stopped and
	 * 0 similarity is returned
	 */
	...
	/**
	 * Invalidates the similarity based distance function, Instances must be still set.
	 */
	public Similarity() {
		invalidate();
	}
	...
	/**
	 * Initialises the similarity based distance function and automatically initialises the
	 * ranges.
	 *
	 * @param data 	the instances the distance function should work on
	 */
	public Similarity(Instances data) {
		setInstances(data);
	}



	public final void setAttributeStats() {
		...
		for (int a = 0; a < m_numAttribs; a++) {
			if (!m_Data.attribute(a).isNominal()) {
				min[a] = (m_Data.attributeStats(a)).numericStats.min;
				max[a] = (m_Data.attributeStats(a)).numericStats.max;
				mean[a] = (m_Data.attributeStats(a)).numericStats.mean;
				attrDifference[a] = max[a] - min[a];
				attrSTD[a] = (m_Data.attributeStats(a)).numericStats.stdDev;
	...
	public String cutoffTipText() {
		...
		+ "When similarity becomes lower than this value,"
		+ "calculations are stopped and 0 similarity is returned"
		+ "(default: 0.0.)";
	}
	...
	/**
	 * sets the cutoff value to calculate the similarity.
	 *
	 * @param co		the new cutoff value
	 * @throws Exception	if instances cannot be processed
	 */
	public void setCutoff(double co) throws Exception {
		cutoff = co;
	}
	...
	public String TNormTipText() {
		return "The TNorm function to calculate the similarity " +
	...
	/**
	 * sets the TNorm to calculate the similarity.
	 *
	 * @param tn		the new TNorm to use
	 * @throws Exception	if instances cannot be processed
	 */
	public void setTNorm(TNorm tn) throws Exception {
		m_TNorm = tn;
	}
	/**
	 * Returns the tip text for this property.
	...
	/**
	 * Gets whether if the attribute values are to be normalized in distance
	...
	public double distance(Instance first, Instance second, double cutOffValue, PerformanceStats stats) {
		...
		for(int i = 0; i< m_Data.numAttributes(); i++){
			if (i != m_classIndex) {
				...
				if (!m_ActiveIndices[i]) continue;

				double mainVal = first.valueSparse(i); // not normalized, the similarity is always normalized
				double otherVal = second.valueSparse(i);

				double sim;

				if (m_Data.attribute(i).isNominal()) {
					if (mainVal==otherVal) sim=1;
					else sim=0;
				}
				else sim = similarity(i, mainVal, otherVal);

				similarity = m_TNorm.calculate(similarity, sim); //t-norm

			}
		}
		distance = 1-similarity; // returns distance, not similarity to the NNsearchalgorithm.

		return distance;
	}


	public abstract double similarity(int index, double first, double second);

	public abstract double[] similarity(int index, double first, double second, double param);
