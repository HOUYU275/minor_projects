...
public abstract class FuzzyRoughRuleInducer   extends AbstractClassifier implements Serializable {
    ...
    public final void initialiseRelation(Relation relation) {
        for (int i=0;i<m_numInstances;i++) {
			for (int j=i;j<m_numInstances;j++) {
				relation.setCell(i,j,1);
			}
		}
    }
	...
	public final void generatePartition(Relation curr, int a) {
		...
		for (int o=0;o<m_numInstances;o++) {
			...
			for (int o1=o+1;o1<m_numInstances;o1++) {
                double sim = fuzzySimilarity(a, mainValue, m_Train.instance(o1).value(a));
				double rel =  m_composition.calculate(sim,curr.getCell(o,o1));
	...
	private void updateCoverage(FuzzyRule rp) {
		for (int o=0;o<m_numInstances;o++) {
			global[o] = m_SNorm.calculate(rp.equivalenceClasses[o],global[o]);
		}
	...
	private final int compare(FuzzyRule r1, FuzzyRule r2) {
		...
		for (int o=0;o<m_numInstances;o++) {
			...
			sum+=m_TNorm.calculate(r1.equivalenceClasses[o],r2.equivalenceClasses[o]);
			r1sum+=r1.equivalenceClasses[o];
			r2sum+=r2.equivalenceClasses[o];
	...
	public void prune() {
		...
		while (it.hasNext()) {
			...
			for (int a = rule.features.nextSetBit(0); a >= 0; a = rule.features.nextSetBit(a + 1)) {
				rule.features.clear(a);
				boolean remove=true;
				
				for (int o=0;o<m_numInstances;o++) {
					double value=1;
					
					for (int b = rule.features.nextSetBit(0); b >= 0; b = rule.features.nextSetBit(b + 1)) {
						value = m_composition.calculate(fuzzySimilarity(b,m_Train.instance(o).value(b),m_Train.instance(rule.instance).value(b)), value);
					}
					if ((float)value!=(float)rule.equivalenceClasses[o]) {
						remove=false;
						break;
					}
				}
				if (!remove) rule.features.set(a);
			}
		}
	}
