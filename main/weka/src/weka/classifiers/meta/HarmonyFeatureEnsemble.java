 ...
 */
public class HarmonyFeatureEnsemble
        extends RandomizableMultipleClassifiersCombiner
        implements TechnicalInformationHandler {

    private int ensembleSize = 10;
    private int poolSize = 50;
    private Classifier[] classifierPool;
    /**
     * for serialization
     */
    ...
    public void fillBaseClassifierPool(String classifierName, Instances data) throws Exception {
        for (int i = 0; i < classifierPool.length; i++) {
            Classifier baseClassifier = AbstractClassifier.forName(classifierName, null);
            FilteredClassifier filteredClassifier = new FilteredClassifier();
            filteredClassifier.setClassifier(baseClassifier);
            Remove rm = new Remove();
            rm.setOptions(new String[]{"-V", "-R", randomSubSpace(data)});
            filteredClassifier.setFilter(rm);
            classifierPool[i] = filteredClassifier;
            classifierPool[i].buildClassifier(data);
        }
    }
    ...
    public void buildClassifier(Instances data) throws Exception {
        ...
        classifierPool = new Classifier[poolSize];
        ...
        for (int i = 0; i < valueRanges.length; i++) {
            valueRanges[i] = new ValueRange(0, poolSize - 1, false);
        }

        ClassifierEnsembleHarmonyMemory harmonyMemory = new ClassifierEnsembleHarmonyMemory(
                HarmonyMemory.createParameterRanges(memorySize, HMCR),
                valueRanges,
                m_Random,
                classifierPool,
                data);
        ...
        int[] selected = harmonyMemory.best().getIntNotes();
        Classifier[] selectedClassifiers = new Classifier[selected.length];
        for (int i = 0; i < selected.length; i++) {
            selectedClassifiers[i] = classifierPool[selected[i]];
        }
...
*/
    ...
    /*public BitSet searchBest(Instances data) throws Exception {
        ...
        harmonyMemory.fill();
        harmonyMemory.iterate(maxIteration);
        BitSet returnBitSet = (BitSet) harmonyMemory.getHarmonyEvaluator(0).translate(harmonyMemory.last());
        //int rank = 0;
        *//*for (Harmony harmony : harmonyMemory) {
            if (harmony == harmonyMemory.first()) {
                System.out.println("FIRST = " + rank);
            }
            else if (harmony == harmonyMemory.last()) {
    ...
    public static void main(String[] argv) {
        runClassifier(new HarmonyFeatureEnsemble(), argv);
    }
    ...
    public String randomSubSpace(Instances data) {
